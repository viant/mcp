...
func (r *RoundTripper) IdToken(ctx context.Context, token *oauth2.Token, metadata *meta2.AuthorizationServerMetadata) (*oauth2.Token, error) {
	...
	if !ok {
		keys, err := meta2.FetchJSONWebKeySet(ctx, metadata.JSONWebKeySetURI, &http.Client{Transport: r.transport})
		if err != nil {
